...
function HarveyCard() {
  const [status, setStatus] = useState<{ online: boolean; error?: string }>({ online: false });
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch("/api/hermes?limit=1", { cache: "no-store" });
        if (!res.ok) {
          const json = await res.json().catch(() => ({}) as Record<string, unknown>);
          setStatus({ online: false, error: (json.error as string) || "not configured" });
          return;
        }
        const data = await res.json() as { status: { online: boolean; lastActivity?: number; pendingCount?: number }; messages: unknown[] };
        setStatus({ online: data.status.online });
        setPending(data.status.pendingCount ?? 0);
        setLastSeen(data.status.lastActivity ?? null);
      } catch {
  ...
  return (
    <Link
      href="/hermes"
      className="flex items-center gap-3 rounded-xl border border-sky-200 bg-sky-50 p-4 shadow-sm transition-colors hover:border-sky-300 hover:bg-sky-50/80 dark:border-sky-800 dark:bg-sky-950"
    >
      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-sky-100 text-base dark:bg-sky-900">
        🤖
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-sm font-medium capitalize text-sky-800 dark:text-sky-100">
          Harvey
        </p>
        <p className="text-xs text-muted-foreground/60">
          {status.error
            ? status.error.includes("not configured")
              ...
              : status.error
            : lastSeen
              ? `Last active ${formatAgo(lastSeen)}`
              : "Hermes agent"}
        </p>
      </div>
